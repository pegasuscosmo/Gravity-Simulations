...
gammaC=1
gammaB=1
c1=ti.Vector([0.9,0.8,0.5]) #high density
c2=ti.Vector([0.9,0.3,0.8]) #mid density
c3=ti.Vector([0.5,0.2,0.7])#low density
midC=0.3
...
@ti.kernel
def init(preset: ti.int32, k: ti.f32, k2: ti.int32):
    if preset==0: #square init
        ...
        for i in range(n):
            ...
            vx=(ti.random()-0.5)*k
            vy=(ti.random()-0.5)*k
            velField[i]=ti.Vector([-d[1],d[0]])/r * ti.sqrt(G*m*n*r/(drawScale/2)**2)*spinMult + ti.Vector([vx,vy])
        ...
        for i in range(n):
            ...
            while r>drawScale/2 and r>0:
                ...
                r=d.norm()
            posField[i]=ti.Vector([x,y])
            velField[i]=ti.Vector([-d[1],d[0]])/r * ti.sqrt(G*m*n/2*r/(drawScale/2)**2)*spinMult
        ...
        for i in range(n):
            ...
            while r>drawScale/4 and r>0:
                ...
                r=d.norm()
            posField[i]=ti.Vector([x,y])
            velField[i]=ti.Vector([-d[1],d[0]])/r * ti.sqrt(G*m*n/2*r/(drawScale/4)**2)*spinMult +ti.Vector([-g*n/100,g*n/100])
...
def findK(L: ti.int32,R: ti.int32) -> ti.int32: #find first differing bit
    ...
    if R<0 or R>=n:
        ...
        if xor==0:
            ...
            for k in range(64):
                bit=63-k
                if (xor>>bit)&1==1 and l==0:
                    i=ti.cast(k,ti.int32) 
                    l=1   
    return i

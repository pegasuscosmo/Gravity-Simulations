...
for n in range(0,bodyCount):
    while True:
        posArr[n]=[random.randint(0,sW*10000)/10000-sW/2,random.randint(0,sW*10000)/10000-sW/2]
...
class quadRect():
    ...
    def insertPoint(self,pointIndex):
        ...
        self.mPosX+=posArr[pointIndex,0]*massArr[pointIndex]
        self.mPosY+=posArr[pointIndex,1]*massArr[pointIndex]
    ...
    def findCOM(self):
        if self.m>0:
            self.comPosX=self.mPosX/self.m
            self.comPosY=self.mPosY/self.m
            if len(self.childList)>0:
                for n in range(0,4):
...
def travelQuad(node,body):
    ...
    while stack:
        ...
        if w/dist<θ or len(children)==0:
            ...
            velArr[body,0]+=accel*dx
            velArr[body,1]+=accel*dy #ndist * (GM/dist**3) ^^^
            continue
        else:
